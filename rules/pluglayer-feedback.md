...
9. Avoid repeatedly submitting the same occurrence in one conversation; new evidence or recurrence warrants a new referenced ticket. Keep feedback-submission failures separate from the original failure.
10. Use `submit_feedback`, `list_my_feedback`, `get_feedback`, or the `share-feedback` skill and `pluglayer-feedback` agent.
